...
// $FlowFixMe
var setPrev = function (node) { return (node._prev = null, node); };
var isPrev = function (node) { return '_prev' in node; };
...
function AmaraPluginDOM(ref            )           {
    ...
    return function createHandler() {
        ...
        var prevTargetDOM                          = new WeakMap();

        function applyDOMToTarget(results                 , target             ) {
            var first = false,
                prevDOM = prevTargetDOM.get(target);
            var currDOM = h('amara:content', (ref = []).concat.apply(ref, results));
            if (!prevDOM) {
                first = true;
                prevDOM = vdomParser(target);
                (ref$1 = currDOM.children).unshift.apply(ref$1, prevDOM.children.map(setPrev));
            } else {
                (ref$2 = currDOM.children).unshift.apply(ref$2, prevDOM.children.filter(isPrev));
            }
            currDOM.tagName = prevDOM.tagName;
            currDOM.properties = prevDOM.properties;
            prevTargetDOM.set(target, currDOM);
            var difference = diff(prevDOM, currDOM);
...
return AmaraPluginDOM;

})));
//# sourceMappingURL=amara-plugin-dom.umd.js.map
